"""
Schemas para Atas de Registro de Preços
"""
...
class AtaItemBase(BaseModel):
    """Schema base para item da ata"""
    ...
    quantidade_registrada: Decimal = Field(..., gt=0, description="Quantidade registrada")
    ...
    def validate_valor_total(cls, v, values):
        ...
        if 'quantidade_registrada' in values and 'valor_unitario' in values:
            calculated = values['quantidade_registrada'] * values['valor_unitario']
            if abs(calculated - v) > Decimal('0.01'):
                raise ValueError('Valor total não confere com quantidade x valor unitário')
        return v


class AtaItemCreate(AtaItemBase):
    """Schema para criação de item da ata"""
    pass


class AtaItemUpdate(BaseModel):
    """Schema para atualização de item da ata"""
    ...
    quantidade_registrada: Optional[Decimal] = Field(None, gt=0)
...
class AtaItemResponse(AtaItemBase):
    """Schema para resposta de item da ata"""
    id: int
    ata_id: int
    criado_em: datetime
...
class AtaBase(BaseModel):
    """Schema base para ata"""
    numero_ata: str = Field(..., max_length=100, description="Número da ata")
    ...
    situacao: str = Field(..., max_length=50, description="Situação da ata")
    objeto_ata: str = Field(..., max_length=2000, description="Objeto da ata")
    valor_total_homologado: Decimal = Field(..., gt=0, description="Valor total homologado")
    ...
    gerente_nome: Optional[str] = Field(None, max_length=200, description="Nome do gerente")
    gerente_cpf: Optional[str] = Field(None, max_length=11, description="CPF do gerente")
...
class AtaCreate(AtaBase):
    ...
    itens: List[AtaItemCreate] = Field(..., min_items=1, description="Lista de itens da ata")


class AtaUpdate(BaseModel):
    """Schema para atualização de ata"""
    ...
    objeto_ata: Optional[str] = Field(None, max_length=2000)
    valor_total_homologado: Optional[Decimal] = Field(None, gt=0)
    ...
    gerente_nome: Optional[str] = Field(None, max_length=200)
    gerente_cpf: Optional[str] = Field(None, max_length=11)


class AtaResponse(AtaBase):
    """Schema para resposta de ata"""
    id: int
    pncp_id: str
    criado_em: datetime
    atualizado_em: datetime
    sincronizado_em: Optional[datetime] = None
    itens: List[AtaItemResponse] = []
    
    class Config:
        from_attributes = True


class AtaFilter(BaseModel):
    """Schema para filtros de ata"""
    numero_ata: Optional[str] = Field(None, max_length=100)
...
class AtaListResponse(SuccessResponse):
    """Schema para resposta de lista de atas"""
    data: PaginatedResponse[AtaResponse]


class AtaDetailResponse(SuccessResponse):
    """Schema para resposta de detalhe de ata"""
    data: AtaResponse


class AtaStatsResponse(SuccessResponse):
    """Schema para resposta de estatísticas de ata"""
    data: dict = Field(..., description="Estatísticas da ata")


class AtaExportRequest(BaseModel):
    """Schema para requisição de exportação de ata"""
    formato: str = Field(..., pattern="^(csv|excel|pdf)$", description="Formato de exportação")
    filtros: Optional[AtaFilter] = None
